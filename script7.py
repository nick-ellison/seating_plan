...
def arrange_seating(people):
    ...
    while attempts > 0:
        ...
        # Ensure even distribution of people across tables
        seating1_males = males[:len(males) // 2]
        seating2_males = males[len(males) // 2:]
        seating1_females = females[:len(females) // 2]
        seating2_females = females[len(females) // 2:]

        seating1 = alternate_gender_seating(seating1_males, seating1_females, len(males) // 2 + len(females) // 2)
        seating2 = alternate_gender_seating(seating2_males, seating2_females, len(males) // 2 + len(females) // 2)

        seating1 += alternate_gender_seating(seating2_males, seating2_females, len(seating2_males) + len(seating2_females) - len(seating1))
        seating2 += alternate_gender_seating(seating1_males, seating1_females, len(seating1_males) + len(seating1_females) - len(seating2))
        ...
        total_wants_score = count_wants_score(seating1) + count_wants_score(seating2)
        total_must_not_violations = count_must_not_violations(seating1) + count_must_not_violations(seating2)
...
def plot_tables(table1, table2):
    ...
    theta = np.linspace(0, 2*np.pi, len(table1) + 1)
    x = 1.75 * np.cos(theta)
    y = 1.75 * np.sin(theta)
...
def output_to_dataframe(table1, table2):
    ...
    table1.extend([None] * (max_len - len(table1)))  # Padding table1 to match the length of table2
    table2.extend([None] * (max_len - len(table2)))  # Padding table2 to match the length of table1
    
    data = {
        'Table 1': [f"{person['Name']} ({person['Gender']})" if person else '' for person in table1],
        'Marital Status 1': [person['Marital_Status'] if person else '' for person in table1],
        'Table 2': [f"{person['Name']} ({person['Gender']})" if person else '' for person in table2],
        'Marital Status 2': [person['Marital_Status'] if person else '' for person in table2],
    }
...
# Ensure there are enough people
if len(people) < 4:
